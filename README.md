...
Not (yet) ported: degen particles + shake, orderbook labels, line/candle morph + live candle animation, window-change transition, loading→data reveal morph, left-edge fade, built-in window/mode/series toggle controls (build these in SwiftUI/UIKit around the view).
